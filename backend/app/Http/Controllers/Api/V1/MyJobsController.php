...
class MyJobsController extends Controller
{
    public function __construct(
        ...
        private readonly MyJobsAttachmentLinker $linker,
    ) {
    }
    ...
    public function linkAttachments(Request $request): JsonResponse
    {
        $validated = $request->validate([
            'dry_run' => ['sometimes', 'boolean'],
        ]);
        ...
        $result = $this->linker->link((bool) ($validated['dry_run'] ?? false));

        $message = $result['dry_run']
            ? 'Dry run complete — no files were linked.'
            : 'MyJobs attachments linked to applications.';
    ...
    public function export(Request $request): StreamedResponse
    {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($row['mailbox_applications'] ?? [] as $application) {
                $mailboxRefs[] = $application['application_reference'] ?? '';
            }
            foreach ($row['myjobs_applications'] ?? [] as $application) {
                $myjobsRefs[] = $application['application_reference'] ?? '';
            }
            ...
            $excelRows[] = [
                ...
                implode('; ', array_filter($mailboxRefs)),
                implode('; ', array_filter($myjobsRefs)),
            ];
        }

        return (new NckReportExcel(
            'MyJobs applicants',
            count($excelRows).' row(s) · MyJobs + email vs MyJobs only',
        ))
